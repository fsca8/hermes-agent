...
_SKILL_REVIEW_PROMPT = (
    "Review the conversation above and update skills if genuinely valuable. "
    "'Nothing to save.' is the DEFAULT — only act when real reusable knowledge "
    "emerged.\\n\\n"
    "CRITICAL RULE: You may ONLY create new skills in PROJECT .hermes/skills/ "
    "directories. If the conversation references a project directory "
    "(e.g. ~/works/xxx), the system will automatically route writes there. "
    "If NO project context is detected, you must NOT create new skills — "
    "only patch or add support files to EXISTING skills.\\n\\n"
    "Signals that warrant action:\\n"
    "  • User corrected your style, tone, format, or workflow — embed the "
    "lesson in the relevant existing skill.\\n"
    "  • A non-trivial technique, fix, or debugging path that is clearly "
    "reusable across sessions emerged.\\n"
    "  • A skill that was loaded or consulted is wrong, missing steps, or "
    "outdated — patch it now.\\n\\n"
    "Preference order:\\n"
    "  1. PATCH a currently-loaded or existing skill (preferred).\\n"
    "  2. ADD support files (references/, templates/, scripts/) under an "
    "existing umbrella.\\n"
    "  3. CREATE a new project-level skill ONLY if a project is active and "
    "no existing skill covers the class.\\n\\n"
    "Protected skills (bundled/hub-installed): DO NOT edit. If the only "
    "skills that need updating are protected, say 'Nothing to save.' and stop.\\n\\n"
    "Say 'Nothing to save.' and stop when:\\n"
    "  • No reusable knowledge was produced (one-off tasks, simple queries).\\n"
    "  • The session ran smoothly with no corrections or techniques to capture.\\n"
    "  • No project context is present and no existing skill needs patching."
)
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • User corrected your style, tone, format, or workflow — embed the "
    "lesson in the relevant existing skill.\\n"
    "  • Non-trivial technique, fix, or debugging path emerged and is "
    "clearly reusable across sessions.\\n"
    ...
    "  3. CREATE a new project-level skill ONLY if a project is active and "
    "no existing skill covers the class.\\n\\n"
    "Protected skills (DO NOT edit): Bundled (hermes-agent etc.) and "
    "hub-installed skills. If the only skills needing updates are protected, "
    ...
    "  • Session-specific transient errors or one-off task narratives.\\n\\n"
    "Act on whichever of the two dimensions has real signal. If "
    "genuinely nothing stands out on either, say 'Nothing to save.' and stop."
)
...
def _detect_project_skills_dir(messages_snapshot: List[Dict]) -> Optional[str]:
    """Scan conversation messages for project paths with .hermes/skills/.

    Looks for patterns like ``~/works/<project>``, ``/c/Users/<user>/works/<project>``,
    or ``C:\\Users\\<user>\\works\\<project>`` in user messages. Returns the
    absolute path to the project's ``.hermes/skills/`` directory if it exists,
    or None.
    """
    home = os.path.expanduser("~")
    project_paths = set()

    # Patterns to detect project mentions in messages
    # 1. ~/works/<project>  or ~/<project>
    # 2. /c/Users/.../works/<project>
    # 3. C:\Users\...\works\<project>
    patterns = [
        re.compile(r"(?:^|\s|`)(?:~|" + re.escape(home) + r")/(?:works/)?([^/\s'\"`)}]+)"),
        re.compile(r"(?:^|\s|`)(?:/c|C:)/(?:Users/[^/]+)/works/([^/\s'\"`)}]+)"),
    ]
    ...
    for msg in messages_snapshot:
        ...
        for pat in patterns:
            for m in pat.finditer(text):
                proj_name = m.group(1).strip().rstrip("/")
                ...
                for candidate in (
                    os.path.join(home, "works", proj_name),
                    os.path.join(home, proj_name),
                ):
                    project_paths.add(candidate)

    # Check each candidate for .hermes/skills/
    for candidate in project_paths:
        skills_dir = os.path.join(candidate, ".hermes", "skills")
